...
def getResult(sQuery,sdbLabel):
    ...
    try:
        ...
        if(re.search(r"\binsert\b",sQuery,re.IGNORECASE)):
            ...
            bMalFormed = True
        elif(re.search(r"\bupdate\b",sQuery,re.IGNORECASE)):
            conn = pyodbc.connect(sConn)
            try:
                cur = conn.cursor()
                result = cur.execute(sQuery).rowcount
                cur.commit()
                print('The Update query ran successfully updating {} of rows and the query executed was {}'.format(str(result),str(sQuery)))
                objDFSkeleton=GetDataFrameSkeleton(ROWCOUNT)
                objDfResult = objPnd.DataFrame([result], columns=objDFSkeleton.columns)
                #print(objDfResult)
                #objDfResult='True'
            except:
                bMalFormed = True			
        ...
        for i in range(qryConf.__len__()):
            ...
            if(bBuildQry==True):
                ...
                if(dfResult.__len__()>0):
                    ...
                    #elif(str(dfResult).__len__() == 4):
                    #    globalstore.globalLogger.info('Updated')
                    else:
